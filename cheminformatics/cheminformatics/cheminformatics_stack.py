...
class CheminformaticsStack(Stack):
    ...
    def _create_megamolbart_service(self):
        ...
        init_container = task_definition.add_container(
            ...
            image=ecs.ContainerImage.from_registry(
                self.node.try_get_context("megamolbart_init_container")
            ),
            ...
            entry_point=["cp", "-R", "-u", "-p", "/megamolbart", "/models/megamolbart"],
            essential=False,
        )
        ...
        # HACK: To fix incorrect CloudFormation names generated by aws_cdk as of v2.3.0
        task = megamolbart.task_definition.node.default_child
        task.add_property_override('Volumes.0.EFSVolumeConfiguration', {'FilesystemId': self.efs_volume_configuration.file_system_id})
        task.add_property_deletion_override('Volumes.0.EfsVolumeConfiguration')
    ...
    def _create_cuchem_service(self):
        ...
        # HACK: To fix incorrect CloudFormation names generated by aws_cdk as of v2.3.0
        task = cuchem.task_definition.node.default_child
        task.add_property_override('Volumes.0.EFSVolumeConfiguration', {'FilesystemId': self.efs_volume_configuration.file_system_id})
        task.add_property_deletion_override('Volumes.0.EfsVolumeConfiguration')
